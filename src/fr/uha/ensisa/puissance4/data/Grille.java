...
public class Grille {
	...
	public double evaluer(Case symboleJoueurCourant) {
		double valeurJoueurCourant = 0;

		double valeurH = evaluerHorizontalement(symboleJoueurCourant);
		if (valeurH == Integer.MAX_VALUE)
			return valeurH;
		double valeurV = evaluerVerticalement(symboleJoueurCourant);
		if (valeurV == Integer.MAX_VALUE)
			return valeurV;
		double valeurD = evaluerDiagonales(symboleJoueurCourant);
		if (valeurD == Integer.MAX_VALUE)
			return valeurD;

		valeurJoueurCourant = valeurH + valeurV + valeurD;
		return valeurJoueurCourant;
	}
	...
	private double evaluerHorizontalement(Case symboleJoueurCourant) {
		double valeurHorizontale = 0;
		int nbAlignes = 0;
		for (int i = 0; i < Constantes.NB_LIGNES; i++) {
			for (int j = 0; j < Constantes.NB_COLONNES; j++) {
				if (grille[j][i] == symboleJoueurCourant)
					...
					if (nbAlignes > 1) {
						valeurHorizontale += nbAlignes;
						nbAlignes = 0;
					}
				}
				// si on a gagné sur cet état on sort de la méthode et on
				// retourne un max
				if (nbAlignes == 4) {
	...
	private double evaluerVerticalement(Case symboleJoueurCourant) {
		double valeurVerticale = 0;
		int nbAlignes = 0;
	...
	private double evaluerDiagonales(Case symboleJoueurCourant) {
		double valeurDiagonale = 0;
		int nbAlignes = 0;
		// calcul alignements diagonaux (bas-droite vers haut-gauche)
		for (int i = 0; i < Constantes.NB_LIGNES - 3; i++)
			for (int j = 0; j < Constantes.NB_COLONNES - 3; j++) {
				for (int x = 0; i + x < Constantes.NB_LIGNES && j + x < Constantes.NB_COLONNES; x++) {
					if (grille[j + x][i + x] == symboleJoueurCourant)
		...
		// calcul alignements diagonaux (bas-gauche vers haut-droit)
		for (int i = 0; i < Constantes.NB_LIGNES - 3; i++)
			for (int j = Constantes.NB_COLONNES - 1; j >= 3; j--) {
				for (int x = 0; i + x < Constantes.NB_LIGNES && j - x >= 0; x++) {
					if (grille[j - x][i + x] == symboleJoueurCourant)
